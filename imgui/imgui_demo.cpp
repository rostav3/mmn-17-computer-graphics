...
void ImGui::ShowDemoWindow(bool *p_open) {
	...
	if (ImGui::CollapsingHeader("Widgets")) {
		...
		if (ImGui::TreeNode("Data Types")) {
			...
			const ImS64 s64_zero = 0, s64_one = 1, s64_fifty = 50, s64_min = LLONG_MIN / 2, s64_max = LLONG_MAX / 2, s64_hi_a =
		...
		if (ImGui::TreeNode("Drag and Drop")) {
...

			{
				...
				static const char *names[9] = {"Bobby", "Beatrice", "Betty", "Brianna", "Barry", "Bernard", "Bibi", "Blaine",
											   "Bryn"};
	...
	if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
		...
		ImGui::SameLine();
		ShowHelpMarker(
				"Enable resizing of windows from their edges and from the lower-left corner. This requires (io.BackendFlags & ImGuiBackendFlags_HasMouseCursors) because it needs mouse cursor feedback.");
		...
		switch (data->EventFlag) {
			...
			case ImGuiInputTextFlags_CallbackHistory: {
				...
				if (prev_history_pos != HistoryPos) {
					data->CursorPos = data->SelectionStart = data->SelectionEnd = data->BufTextLen = (int) snprintf(data->Buf,
																													(size_t) data->BufSize,
																													"%s",
																													(HistoryPos >=
																													 0)
